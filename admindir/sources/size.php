...
switch ($act) {
    case "edit":
        ...
        $sql_size = "SELECT * FROM {$GLOBALS['db_sp']}.size WHERE id = ?";
        $footer = $GLOBALS['sp']->getRow($sql_size, [$id]);
        $smarty->assign('edit', $size);
        $template = "size/edit.tpl";
        break;

    case "add":
        $template = "size/create.tpl";
        break;
    ...
    case 'dellistajax':
        ...
        if ($ids !== '') {
            ...
            $GLOBALS["sp"]->query("DELETE FROM {$GLOBALS['db_sp']}.size WHERE id IN ($idList)");
    ...
    case "editsm":
        saveColor();
        page_transfer2("index.php?do=size");
        exit;

    default:

        $size = $GLOBALS["sp"]->getAll("SELECT * FROM {$GLOBALS['db_sp']}.size order by num desc");
        $smarty->assign("view", $size);
        $template = "size/list.tpl";
        break;
}
...
function saveColor()
{
    ...
    $newNum = ($act === 'addsm')
        ? (($sp->getOne("SELECT MAX(num) FROM {$GLOBALS['db_sp']}.size") ?: 0) + 1)
